...
description: 前端总入口。用于当前 plus-ui-react 项目中的标准 React CRUD 页面、新增 API/types、复杂列表页增强、树筛选、导入导出、权限按钮、状态切换、弹窗表单和 React 运行时代码生成模板维护任务，并根据任务类型选择合适的前端子 agent。
---
...
4. 如果是维护代码生成模板，先修改后端 `backend/ruoyi-admin/ruoyi-gen/src/main/resources/fm/react/` 运行时事实源，再同步当前项目 `gen/` 镜像。
...
- 冲突时优先相信当前项目真实页面，其次是公共组件、hooks、工具和请求封装，再其次是本仓库后端 `fm/react/*.ftl` 运行时模板，最后才是 Vue 和外部上游参考。
...
- `fm/react/index.tsx.ftl`、`fm/react/index-tree.tsx.ftl` 及对应 `gen/` 镜像必须生成 React TSX 页面。
