...
开发环境会按运行平台选择 Client API：Android 模拟器使用 `http://10.0.2.2:8082`，iOS 模拟器使用 `http://localhost:8082`。真机联调时应把开发地址配置为局域网可访问的 Client Gateway；复制脚手架创建产品时，必须将 `src/utils/env.ts` 中的生产占位域名替换为该产品的 HTTPS Client Gateway 域名。
