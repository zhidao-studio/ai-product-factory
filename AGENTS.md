...
本文是 `ai-product-factory` 的 Agent 快速入口。完整工程约束、接口契约和运行说明见 [CLAUDE.md](./CLAUDE.md)；当前完成度、业务留白与角色化阅读路径见 [docs/工程现状.md](./docs/工程现状.md)；从需求到实施的门禁见 [docs/需求驱动开发流程.md](./docs/需求驱动开发流程.md)。
...
1. 真实业务改动先按 Spec Kit 流程形成 `spec.md`；Spec 只写 WHY / WHO / WHAT、场景、业务规则、验收和非目标，禁止夹带技术方案。Spec、Plan、Tasks 分别经人工确认，Analyze 消解重大问题并获得明确实施授权后，才能改业务代码。
2. 生成或修改 UI 前，完整读取 `docs/AI-设计系统上下文.md` 和对应的 `docs/平台适配/<端>.md`；Token 是唯一样式来源。
3. 前端契约必须以后端真实 Controller 与 VO 为准，禁止发明接口路径或返回字段。
4. 后端沿用 RuoYi 的 `controller / service / mapper / domain.bo / domain.vo`、MyBatis-Plus、`R<T>` 与现有认证策略写法，禁止另造 DDD/Port/JdbcTemplate 体系。
5. Admin 与 Client 的业务模块和 API 契约分别由各自总工程拥有；Common 不得定义业务 API，也不得依赖任何一侧的业务实现。
6. 新增模块必须加入其所属总工程的 `pom.xml`；不要提前创建无实现的 API、UM 或微服务空壳。
7. 新增 Client 表统一使用 `app_*`；表七要素固定为 `id`、`valid_flag`、`del_flag`、`create_by`、`create_time`、`update_by`、`update_time`，不含部门和通用乐观锁版本。Admin 的 `sys_*` 表继续沿用原 RuoYi 字段，两套表允许存在结构差异。
8. Admin 调用 Client 的 `/internal/admin/**` 只允许后端私有网络访问，必须使用独立服务签名与防重放校验；内部审计只传并签名操作人 ID，不传 Admin 部门 ID，禁止经过任一 Gateway 或转发浏览器 Token。
9. 主运行时、主框架和主语言版本必须遵循 [`docs/工程版本基线.md`](./docs/工程版本基线.md)：生产运行时采用最新 LTS 补丁，框架/编译器采用最新 GA；上游尚未兼容时只能采用有依据、可复核的最新兼容稳定版，禁止预发布版本和机械追 `latest`。
...
- 设计系统：`docs/`
- 需求工件：`specs/`；项目约法与模板：`.specify/`；本地校验：`node scripts/verify-spec-kit.mjs`、`node .specify/scripts/verify-specs.mjs`；双 Agent 重建：`bash scripts/sync-spec-kit-integrations.sh`
- Codex Spec Kit Skills：`.agents/skills/speckit-*/`；Claude Code Skills：`.claude/skills/speckit-*/`
...
- 开始真实业务需求：先阅读 `docs/需求驱动开发流程.md`，从 Specify 开始；不用一键 workflow 跳过人工门禁。
